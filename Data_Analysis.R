...
# Moderate correlation btwn Total Sales and Per Capita Sales on Zipcode and County.
# 
# What this means. Although there are a lot of people in cities it doesn't look
# like many are buying. Better to target via Zipcode and County. 
...
abline(h=0, col='red')
# NOTE: Our residuals may look nice at first glance but we can see we are
# actually predicting higher values than actual and the y axis is on a scale 
# hundreds of thousands. This is likely because all cities that have a singular
# zipcode within our data set will show up as having equal avg sales. This 
# would then also heavily bias our correlation coefficient if there were many
# instances of this which it appears is the case. 
...
# NOTE: Whiskey returns NA, likely because there was a city that did not have 
# data for whiskey. We could go looking for it and/or create a loop to exclude
# that city, BUT by the correlation coefficients for the rest of the categories
# we can assume that whiskey follows in suit. 
#
# We can see as population increases so does your total number of sales which
# makes sense.


##############################################################################
# PART 2 of the Research Question
# Does the avg sales per zipcode and per corresponding pattern differ across
# liquor categories
##############################################################################
